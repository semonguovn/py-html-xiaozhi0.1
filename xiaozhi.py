...
TTS_CONFIG = {
    "appid": "appid", #appid
    "access_token": "access_token", #写自己的
    "cluster": "volcano_icl",
    "voice_type": "S_", #这个是自己生成的声音ID
...
def on_message(client, userdata, message):
    global aes_opus_info, udp_socket, tts_state, recv_audio_thread, send_audio_thread, last_message
    ...
    if msg['type'] == 'tts':
        tts_state = msg['state']
        # 只添加这一行，处理 tts 文本生成音频
        if msg['state'] == 'sentence_end' and 'text' in msg:
            msg['audio_url'] = handle_tts_text(msg['text'])
...
async def generate_tts(text):
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer;{TTS_CONFIG['access_token']}"}
...
def handle_tts_text(text):
    try:
        # 使用asyncio运行异步函数
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        audio_filename = loop.run_until_complete(generate_tts(text))
        loop.close()
